...
// Import domain mock fallback data with correct export bindings
  ...
  const fetchThreads = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        // Normalize messages array so it is never undefined
...
console.log('Fixed lib/use-api.ts mock imports and useThread/usePipeline normalization');

// 2. Update lib/api-client.ts with robust 401 retry validation
let apiClientContent = fs.readFileSync(path.join(rootDir, 'lib', 'api-client.ts'), 'utf8');
...
fs.writeFileSync(path.join(rootDir, 'lib', 'api-client.ts'), apiClientContent, 'utf8');
console.log('Fixed lib/api-client.ts 401 retry validation');
...
if (!layoutContent.includes('AuthProvider')) {
  ...
  console.log('Mounted AuthProvider in app/layout.tsx');
}

console.log('✅ ALL PHASE 5 REVIEWS RESOLVED!');
`;

fs.writeFileSync(path.join(rootDir, 'scripts', 'apply-p5-fixes.js'), useApiContent, 'utf8');
